...
public class Practice9 {
    ...
    static class Book {
        ...
        // 생성자 : 객체 초기화/만들 때. 생성할 때 사용되는 메소드
            Book(String title, String author, int price) {
                ...
                this.author = author;
                this.price = price;
            } // 클래스 내에 오른쪽 마우스의 오른쪽(빈공간)클릭 -> 생성-> 생성자 -> 매개변수선택(복수) -> 확인
        }
    ...
    static class Rectangle{
        ...
        Rectangle( int width, int height ){// 생성자
            this.width = width;     this.height= height; // 멤버변수
        }
        int getArea(){ // 메소드
    ...
    static class Goods{
        ...
        // 2. 일반 생성자
        Goods( String name, int price){ // 오버로드 : 동일한 생성자명으로 매개변수에
            this.name = name;
            this.price = price;
        }
    }
    ...
    2. main 함수에서 new Member()로 객체를 생성하고, 초기화된 id와 isLogin 값을 출력하여 확인하세요.*/
        ...
        System.out.println("member.id = "+ member1.id);
        System.out.println("member.isLogin"+ member1.isLogin);
