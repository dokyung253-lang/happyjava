...
public class Exam1 {
    public static void main(String[] args) {
        ...
        for(int 곱 = 1; 곱 <= 9 ; 곱++){System.out.println(곱);}
           // (3) 단 마다 곱?? 곱마다 단?? >> 기존의 구구단 외울 때 단부터  vs 구매목록 출력하고 제품목록[pno(식별번호)/pname(식별명)]
        for(int 단 = 2 ; 단 <= 9; 단++) {
            for(int 곱 = 1;  곱 <= 9; 곱++){
        ...
        for(int i = 1; i <= 5; i++){ // 코드 흐름이 break만나면 반복문 탈출
            if(i==3){break;}
            System.out.println(i);
        } // 1 2

        // [3] continue : 가장 가까운 반복문(증감식) 이동;
        for(int i=1; i<=5; i++){
            if(i==3){continue;} // 코드 흐름이 continue 만나면 증감식(i++)으로 이동
            System.out.println(i);
        }// 1 2 3 4

        // [4] 무한루프 : 조건문이 true 상수로 계속되는 반복문
        //for( ; ; ){ System.out.println("무한루프"); }
        while (true){ // 조건식에 true상수를 넣으면 무한루프
