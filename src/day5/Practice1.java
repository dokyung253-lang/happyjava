...
public class Practice1 {
    ...
    Arrays.toString()을 사용하여 배열의 모든 요소를 한 번에 출력하세요.*/
        ...
        season[0] = "봄";
        season[1] = "여름";
        season[2] = "가을"; // (2) 각 인덱스의 요소/값 대입, push 기능 없다.
    ...
    선언 코드: 하단*/
            ...
            for (int index = 0; index <= products.length - 1; index++) {
                if (products[index].equals(productName)) { // 만약에 index번째 제품명과 입력받은 제품명과 같으면
                    if (stock[index] >= stockCount) {// 만약에 index번째 재고가 입력받은 재고보다 이상이면
                        stock[index] -= stockCount; // 입력받은 재고만큼 index번째 재고 차감한다.
                        check = 1;
                        break; // 1이면 '구매완료' 뜻
                    }// if end
                    else {
            ...
            if (check == 0) {
                System.out.println("없는 제품입니다."); // for 밖에서 (끝나고) 제품없음을 출력한다.
            } else if (check == 1) {
...
931나8234: 420분 주차, 최종 요금: 20000원*/
            ...
            int money = 0; // 1) money요금 변수 선언
            // 1) money 요금 변수 선언
            for (int index = 0; index <= carNumbers.length - 1; index++) {
                String car = carNumbers[index]; //index번째 차량번호 호출
                System.out.print(car);
                int useTime = usageMinutes[index]; // index번쨰 사용시간 추출
                if (useTime <= 30) {
                    money = 1000;
                } else {
