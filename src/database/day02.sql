...
create table waiting(
	count tinyint unsigned, -- 인원수 최대 정수 255 까지, signed +-128
	phone char(13) -- 연락처 최대 13글자 까지 고정길이 문자
);
select * from waiting;
drop table if exists test5;
drop table if exists test4;
-- [6] 제약조건 : 테이블 내 데이터들의 문제 일으키는 입력 방지/ 해결
create table test4(
	# 속성명 타입 제약조건명
    ...
    속성명4 bigint auto_increment, 	-- auto-increment : 만일 해당 속성에 초기값이 생략되면 자동번호 부여한다. + pk값이랑 같이 사용한다.
	constraint primary key(속성명4) 	-- primary key : pk(식별/기본)키, 식별가능한 고유한 값 가진 키
									  -- {중복없음 unique + 빈칸없음 not null} 기능 포함 예) 학번,사번,주민등록번호,제품번호 등
...
create table test5(
    ...
    -- foreign key : fk(참조/외래)키, 다른 테이블의 기본키(pk)를 참조하는 키 
    -- constraint foreign key(FK속성명) references 테이블명 (pk필드명)		
    -- 중복 가능, 빈칸 가능
	-- pk(1) : fk(n) 관계에서, fk : 수강신청한학번, 휴가신청한사번, 구매한제품번호 등등
    -- on update/delate restrict : pk 참조관계일 때 fk가 *참조중*이면 삭제/수정 불가능 관계 <자식이 있으면 부모 삭제 불가능> , 기본값
				-- cascade : pk가 삭제/수정 될 때 fk도 같이 삭제 관계, <게시물 삭제되면 댓글도 같이 삭제>
                -- set null : pk가 삭제/수정 될 때 fk는 null 변경 관계, <게시물 삭제되면 댓글 참조는 null>
); 
select * from test5;
-- [5] 회원제 제품 설계 -- 관례적인 순서
...
create table member(
    ...
    constraint primary key(mno) -- 회원번호를 pk(식별키) 설정
);
select * from member;
create table product(
	pno int auto_increment , -- 자동번호 부여 *자주쓰임
    ...
    pdate datetime default now(), -- 날짜/시간타입, 기본값을 현재시스템 자동 부여 *자주쓰임
    mno int, -- Fk로 사용할 필드명의 타입은 PK 필드명과 일치, 필드/속성명 일치 권장		-- on delete cascade 회원이 탈퇴하면 그 회원 제품도 삭제
