...
# [1] INSERT ; 레코드 , 행, 튜플 : 1줄 삽입 / DTO 1개 삽입
...
-- [2] select, 레코드 조회, 주의할 점: 조회 결과테이블과 원본테이블 상관없다.
# 1) select * from 테이블명;
select * from test;
...
select * from test where mname = "유재석"; -- SQL 에서는 = 같다 기호로 사용된다. VS = 대입
...
set SQL_SAFE_UPDATES = 0; -- 실무에서 수정과 삭제는 위험하므로 workbench에서는 수정/삭제 차단 상태
# 1) update 테이블명 set 수정할속성명1 = 수정할값, 수정할속성명2 = 수정할값 where 조건
update test set mcount = 0; -- where조건이 없으면 전체수정
# 2) 일반적으로 조건수정이다.
update test set mcount = 10 where mname = "유재석"; -- mname의 속성값이 유재석이면 mcount 속성값을 10으로 변경해라.
update test set mcount = 20 , mname = "강호동2" where mno = 2;
-- mno 속성값이 2이면 mcount 속성값을 20, mname 속성값을 '강호동2' 변경해라.

-- [4] delete, 레코드 삭제, delete : 레코드 삭제 VS truncate : 모든레코드 삭제 VS drop : 테이블/데이터베이스 삭제
# 1) delete from 테이블명 where 조건
delete from test where mno = 2; -- mno속성값이 2이면 (강호동레코드) 삭제하라
# 2) 
...
/*
	DDL : 데이터베이스 정의어  , 뒤로가기(롤백 = 트렌잭션) 불가능
		  예] create, drop, truncate
    DML : 데이터베이스 조작어  , 뒤로가기(롤백 = 트렌잭션) 가능
...
# 1] as 별칭 키워드, 조회결과의 속성명 변경 , as 생략 가능하다. , 속성명/테이블명 길거나 복잡한 경우 사용.
...
# 3] 산술연산자 , 주의할 점 : select 결과는 *항상* 표/테이블 단위이다.
...
# 주의할 점 : null(자료 없음) 비교 연산자 별도 존재 ;		is null, is not null
select * from member where mphone1 = null;		-- 비교 불가능; = null (x)
select * from member where mphone1 is null; 		-- 비교 가능;    is nul (o)
select * from member where mphone1 is not null; -- 비교 가능 
# 문자패턴 : 1)  속성명 like '문자패턴'	, 2) % : 문자대응(개수 상관x)	, 3) _ : 문자대응(_개수만큼 대응)
...
select * from member where mname like "%이%"; 	-- '이'가 포함된 문자 	예] contains  (java)
