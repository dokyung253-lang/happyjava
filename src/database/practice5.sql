...
select 제품명 from product; -- 1) 모든 제품의 제품명
select 카테고리명 from pcategory; -- 2) 모든제품의 카테고리명 
-- 3) 서로 다른 테이블 간의 교집합(내부조인) 찾기 : Pk속성명과 Fk 속성명, 카테고리번호_pk = 카테고리번호_fk;
...
select p.제품명, pc.카테고리명 from product p inner join pcategory pc
	on p.카테고리번호_fk = pc.카테고리번호_pk where pc.카테고리명 = '노트북';


-- [문제 3] 모든 제품의 제품명과 등록된 재고수량을 함께 조회하세요. (조회결과: 8개 레코드)
select 제품명 from product; -- 1) 모든제품의 제품명
...
--  group by 사용(이전 수업) - 총합

-- [문제 9] 각 제품별로 총 재고 수량을 조회하고, 총 재고 수량이 많은 순서대로 정렬하여 제품명과 총재고수량을 표시하세요. (조회결과: 6개 레코드)
--  join 후 group by 사용(이전 수업) - 총합
select 제품명, sum(s.재고수량) as 총재고수량
