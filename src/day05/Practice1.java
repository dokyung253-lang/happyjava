...
public class Practice1 {
    ...
    season[0] = "봄"; season[1] = "여름"; season[2] = "가을"; // (2) 각 인덱스의 요소/값 대입, push 기능 없다.
    ...
    int[] scores1 = {85, 92, 78, 65, 95};
        int sum = 0; // 합계 변수
        for(int index = 0; scores1.length <= -1; index++){
    ...
    선언 코드: int[] scores2 = {77, 82, 100, 54, 96};*/
        ...
        for( int score : scores2){ // 향상된 for 문
            if( score == 100){
    ...
    선언 코드: int[] numbers2 = {23, 5, 67, 12, 88, 34};*/
        ...
        for( int value : numbers2 ){ // 향상된 for문
            if( max < value){ // max보다 index번쨰 요소값이 더 크면 대입
        }
            System.out.println("max ="+ max); // sout+v 자동완성
    ...
    Scanner scan = new Scanner(System.in); // 제품명과 재고 입력받아
            ...
            for( int index = 0; index <= products.length -1; index++){
                if( products[index].equals(productName)){ // 만약에 index번째 제품명과 입력받은 제품명과 같으면
                    ...
                    if(stock[index] -= stockCount; // 입력받은 재고만큼 index번째 재고 차감한다.
                    check = 1; break; // 1이면 '구매완료' 뜻
                    }// if end
                    else{check = 2; break;} // 2이면 '재고부족' 뜻
                    }// if end
                }// for end
            if( check == 0){ System.out.println("없는 제품입니다."); // for 밖에서 (끝나고) 제품없음을 출력한다.
                else if( check == 1){ System.out.println("구매완료");
                else if( check == 2){ System.out.println("재고부족");
